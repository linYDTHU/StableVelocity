...
def get_velocity_from_x(x, xt, t, path_type="linear"):
    """Wrapper function: transfrom data prediction model to velocity
    Args:
        x: [batch_dim, ...] shaped tensor; data model output
        xt: [batch_dim, ...] shaped tensor; x_t data point
        t: [batch_dim,] time tensor
    """
    t = expand_t_like_x(t, xt)
    if path_type == "linear":
    ...
    v_t = d_alpha_t * x + d_sigma_t * (xt - alpha_t * x)/sigma_t

    return v_t

def _to_velocity(output, model_input, t, prediction_type="v", path_type="linear"):
    if prediction_type == "v":
        return output
    elif prediction_type == "x":
        return get_velocity_from_x(output, model_input, t, path_type=path_type)
    else:
        raise NotImplementedError
...
def get_model_attr(model, attr, default=None):
    """Safely get attribute from model, handling DDP wrappers."""
    if hasattr(model, attr):
        return getattr(model, attr)
    elif hasattr(model, 'module') and hasattr(model.module, attr):
...
def get_time_discretization(nfes: int, rho=7):
    ...
    t_samples = torch.clip(time_vec, min=0.0, max=1.0)
    return t_samples
...
def euler_sampler(
    ...
    if edm_schedule:
        t_steps = get_time_discretization(num_steps)
    else:
    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            if get_model_attr(model, 'use_projector', False):
                d_cur = _to_velocity(
                    model(
            ...
            if heun and (i < num_steps - 1):
                if cfg_scale > 1.0 and t_next <= guidance_high and t_next >= guidance_low:
                    model_input = torch.cat([x_next] * 2, dim=0)
                    y_cur = torch.cat([y, y_null], dim=0)
                else:
                    model_input = x_next
                    y_cur = y
