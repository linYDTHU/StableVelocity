...
class ConvProjector(nn.Module):
    """
    Conv2d-based projector for iREPA.
    Uses a 3x3 convolution instead of MLP for better spatial preservation.
    """
    def __init__(self, hidden_size, z_dim, spatial_size):
        super().__init__()
        self.spatial_size = spatial_size
        self.hidden_size = hidden_size
        self.conv = nn.Conv2d(hidden_size, z_dim, kernel_size=3, padding=1)
    
    def forward(self, x):
        """
        Args:
            x: Input tensor of shape (N*T, D) where T = spatial_size^2
        Returns:
            Output tensor of shape (N*T, z_dim)
        """
        N_T, D = x.shape
        H = W = self.spatial_size
        T = H * W
        N = N_T // T
        # Reshape from (N*T, D) to (N, D, H, W)
        x = x.view(N, H, W, D).permute(0, 3, 1, 2)  # (N, D, H, W)
        x = self.conv(x)  # (N, z_dim, H, W)
        # Reshape back to (N*T, z_dim)
        x = x.permute(0, 2, 3, 1).reshape(N * T, -1)  # (N*T, z_dim)
        return x


def build_conv_proj(hidden_size, z_dim, spatial_size):
    """Build a Conv2d-based projection layer for iREPA."""
    return ConvProjector(hidden_size, z_dim, spatial_size)
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs # fused_attn
        ...
        self.use_projector = use_projector and (z_dims is not None and all(z > 0 for z in z_dims))
        ...
        if self.use_projector:
            # Compute spatial size for ConvProjector (sqrt of num_patches)
            spatial_size = int(num_patches ** 0.5)
            if use_irepa:
                # iREPA: Use Conv2d-based projector instead of MLP
                self.projectors = nn.ModuleList([
                    build_conv_proj(hidden_size, z_dim, spatial_size) for z_dim in z_dims
                ])
            else:
                # Original REPA: Use MLP projector
                self.projectors = nn.ModuleList([
    ...
    def forward(self, x, t, y=None):
        ...
        for i, block in enumerate(self.blocks):
            x = block(x, c)                      # (N, T, D)
            if self.use_projector and (i + 1) == self.encoder_depth:
